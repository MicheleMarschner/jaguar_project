...
def discover_experiment_groups() -> list[str]:
    if not PATHS.runs.exists():
        return []
    return sorted(p.name for p in PATHS.runs.iterdir() if p.is_dir())

def detect_output_profile(experiment_group: str) -> str | None:
    ...
    for run_dir in sorted(p for p in run_root.iterdir() if p.is_dir()):
        ...
        output_profile = _safe_get(cfg, "output", "profile")
        if output_profile:
...
def aggregate_experiment_outputs(experiment_group: str, output_profile: str) -> Path:
    ...
    run_root = PATHS.runs / experiment_group
    if not run_root.exists():
        raise FileNotFoundError(f"Experiment group not found: {run_root}")
    ...
    if n_run_dirs == 0:
        raise RuntimeError(f"No run directories found under: {run_root}")

    if n_valid_runs == 0:
        raise RuntimeError(
            f"No valid runs with metrics.json found under: {run_root}"
        )
