...
def plot_phash_threshold_diagnostics(
    artifacts_dir: Path,
    save_dir: Path,
    filename: str = "phash_threshold_diagnostics.png",
) -> Path:
    """
    Plot Stage-1 pHash threshold diagnostics:
    left = within-identity links, right = cross-identity collisions.
    """
    ensure_dir(save_dir)

    df = pd.read_parquet(artifacts_dir / "phash_diagnostics.parquet").sort_values("threshold")

    safe_rows = df[df["cross_links"] == 0]
    chosen_threshold = int(safe_rows["threshold"].max()) if len(safe_rows) else None

    fig, axes = plt.subplots(1, 2, figsize=(10, 4))

    # Left: within links
    axes[0].plot(df["threshold"], df["within_links"], marker="o")
    axes[0].set_title("Within-identity links")
    axes[0].set_xlabel("pHash threshold")
    axes[0].set_ylabel("Linked pairs")
    axes[0].grid(alpha=0.3)

    # Right: cross links
    axes[1].plot(df["threshold"], df["cross_links"], marker="o")
    axes[1].set_title("Cross-identity collisions")
    axes[1].set_xlabel("pHash threshold")
    axes[1].set_ylabel("Linked pairs")
    ...
    if chosen_threshold is not None:
        ...
        axes[1].annotate(
            f"chosen = {chosen_threshold}",
            xy=(chosen_threshold, 0),
            xytext=(chosen_threshold + 0.5, max(df["cross_links"]) * 0.15 if df["cross_links"].max() > 0 else 1),
        )
...
def summarize_phash_threshold_choice(artifacts_dir: Path) -> dict:
    df = pd.read_parquet(artifacts_dir / "phash_diagnostics.parquet").sort_values("threshold")
    
    safe_rows = df[df["cross_links"] == 0]
    chosen_threshold = int(safe_rows["threshold"].max())

    chosen_row = df[df["threshold"] == chosen_threshold].iloc[0]
    first_unsafe_row = df[df["cross_links"] > 0].iloc[0]

    print(f"[Stage1] Thresholds evaluated: {df['threshold'].min()} to {df['threshold'].max()}")
    print(f"[Stage1] Sampled within-identity pairs: {int(df['n_within_pairs'].iloc[0])}")
    print(f"[Stage1] Sampled cross-identity pairs: {int(df['n_cross_pairs'].iloc[0])}")
    print(f"[Stage1] Chosen safe threshold: {chosen_threshold}")
    print(
        f"[Stage1] At threshold {chosen_threshold}: "
        f"within_links={int(chosen_row['within_links'])} / {int(chosen_row['n_within_pairs'])}, "
        f"cross_links={int(chosen_row['cross_links'])} / {int(chosen_row['n_cross_pairs'])}"
    )

    return {
        "chosen_threshold": chosen_threshold,
        "within_links_at_chosen": int(chosen_row["within_links"]),
        "n_within_pairs": int(chosen_row["n_within_pairs"]),
        "first_unsafe_threshold": int(first_unsafe_row["threshold"]),
        "cross_links_first_unsafe": int(first_unsafe_row["cross_links"]),
        "cross_link_rate_first_unsafe": float(first_unsafe_row["cross_link_rate"]),
    }
...
def run_burst_analysis(
    ...
    plot_phash_threshold_diagnostics(
        artifacts_dir=artifacts_dir,
        save_dir=save_dir
    )
    summarize_phash_threshold_choice(artifacts_dir)
